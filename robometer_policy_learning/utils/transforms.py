...
@dataclasses.dataclass(frozen=True)
class MinMaxNormalize(DataTransformFn):
    """Normalize data to [-,1] range using min-max scaling."""
    ...
    def _normalize(self, x, max: float, min: float):
        return (x - min) / (max - min + 1e-6)


@dataclasses.dataclass(frozen=True)
class MinMaxUnnormalize(DataTransformFn):
    """Unnormalize data from [0,1] range back to original scale."""
    ...
    def _unnormalize(self, x, max: float, min: float):
        return x * (max - min + 1e-6) + min
...
class Normalize(DataTransformFn):
    ...
    def _normalize_quantile(self, x, stats: dict):
...
def flatten_dict(tree: dict) -> dict:
    ...
    return {k.replace(".", "_"): v for k, v in tree.items()}
...
def _assert_quantile_stats(norm_stats: dict) -> None:
