...
def compute_loss_q(ac: Ensemble, ac_targ: Ensemble, obs, act, obs2, rew, done, gamma: float):
    with torch.no_grad():
        a2 = torch.mean(torch.stack([pi(obs2) for pi in ac.pis]), dim=0)
    q1, q2 = ac.q1(obs, act), ac.q2(obs, act)
    with torch.no_grad():
        backup = rew + gamma * (1 - done) * torch.min(ac_targ.q1(obs2, a2), ac_targ.q2(obs2, a2))
...
def _buffer_parts(buf) -> List[Any]:
    """For breaking down the MixedReplayBuffer."""
...
def q_batch_indices(n: int, batch_size: int, pos_fraction: float,
                    rng: np.random.Generator, pos_pool: np.ndarray) -> np.ndarray:
    """LIBERO success is sparse, so uniform sampling shows the critic almost no positives and it
    learns to predict ~0 everywhere. Forcing 10% positives per batch keeps the critic's output informative.
    """
    if n == 0:
    ...
    neg = rng.integers(0, n, size=batch_size - want_pos)
    if want_pos == 0:
        return neg
...
def train_thrifty_models(algo, ac: Ensemble, ac_targ: Ensemble, ens_opt_fn, q_opt,
    ...
    """One refresh: rebuild + refit the ensemble, then refit the twin critics."""
    ens_losses, q_losses = [], []
    rng = np.random.default_rng(seed)
    buf = algo.buffer
    bs = algo.batch_size
    # One index space over offline demos + online corrections, matching the reference's single
    # replay_buffer. Built once per refresh; the stores stay lazy.
    space = _IndexSpace(buf, device)
    n_t = space.total
    _batch = space.batch
    ...
    for i in range(num_nets):
        boot = bootstrap_indices(n_t, rng)
        if boot.size == 0:
            break
        for _ in range(grad_steps):
            batch = _batch(rng.choice(boot, size=bs))
    ...
    pos_pool = space.positive_indices()
    n_pos = int(len(pos_pool))
    for step in range(grad_steps):
        idx = q_batch_indices(n_t, max(2, bs // 2), POS_FRACTION, rng, pos_pool)
        batch = _batch(idx)
        if not batch or len(batch.get("action", [])) == 0:
            break
        feat = _encode_batch(algo, batch["obs"])
        ...
        loss = compute_loss_q(ac, ac_targ, feat, act, feat2, rew, done, gamma)
        q_opt.zero_grad()
        loss.backward()
        q_opt.step()
        if step % 2 == 0:
...
class ThriftyGate:
    ...
    def recalibrate(self, novelties: np.ndarray, safeties: np.ndarray, target_rate: float):
        # Recalibrate both thresholds every iteration 
        n = len(novelties)
        if n == 0:
            return
    ...
    def recalibrate_online(self, target_rate: float, min_samples: int = 25) -> bool:
        # Recalibrate after every episode
...
class ThriftyScorer:
    def __init__(self, algo, ac: Ensemble, remove_obs_keys=None, device=None):
        self.algo = algo
        self.actor = algo.actor
        self.ac = ac
    ...
    def _actor_obs(self, obs):
        ...
        prepped = {k: v for k, v in obs.items() if k not in self.remove_obs_keys}
    ...
    def score(self):
        ...
        safety = float(self.ac.safety(f, a.cpu().numpy()))
