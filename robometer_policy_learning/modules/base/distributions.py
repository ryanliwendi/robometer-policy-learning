...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution(
        ...
        # Clamp std to avoid numerical issues (very small or NaN std)
        action_std = th.clamp(action_std, min=1e-6)
...
class MultiCategoricalDistribution(Distribution):
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        ...
        return th.stack(
            [dist.log_prob(action) for dist, action in zip(self.distribution, th.unbind(actions, dim=-1))],
            dim=-1,
        ).sum(dim=-1)
...
def kl_divergence(dist_true: Distribution, dist_pred: Distribution) -> th.Tensor:
    ...
    if isinstance(dist_pred, MultiCategoricalDistribution):
        ...
        assert np.allclose(dist_pred.action_dims, dist_true.action_dims), (
