"""VENDORED VERBATIM from ThriftyDAgger (Hoque et al., CoRL 2021).

Source: https://github.com/ryanhoque/thriftydagger -- thrifty/algos/core.py
Unmodified. Do not edit: any change here breaks the guarantee that the baseline's networks
and its novelty/safety statistics are the authors' own. The only coupling to their env is
`observation_space.shape[0]` / `action_space.shape[0]` / `action_space.high[0]`, which we
satisfy with a small shim (see robometer_policy_learning/utils/thrifty_gate.py).
"""
